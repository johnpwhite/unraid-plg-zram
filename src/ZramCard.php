    ...
    function getZramDashboardCard() {
        ...
        try {
            ...
            if ($ssdPath && file_exists($ssdPath)) {
                // Resolve swap target: loop device for loop-backed, resolved
                // /dev/zdN alias for zvol-backed, path for file-backed. See
                // docs/specs/TIER2_ZVOL_SWAP.md for the zvol resolution.
                $swapTarget = $ssdPath;
                if ($ssdBacking === 'loop') {
                    $ljOut = [];
                ...
                } elseif ($ssdBacking === 'zvol') {
                    $resolvedZvol = realpath($ssdPath) ?: '';
                    if ($resolvedZvol !== '') $swapTarget = $resolvedZvol;
                }
                $swaps = @file_get_contents('/proc/swaps') ?: '';
                $ssdActive = preg_match('/^' . preg_quote($swapTarget, '/') . '\s/m', $swaps) === 1;
                if ($ssdActive) {
                    // Show the resolved device name when loop/zvol-backed so
                    // it's not mistaken for a plain file.
                    if (($ssdBacking === 'loop' || $ssdBacking === 'zvol') && $swapTarget !== $ssdPath) {
